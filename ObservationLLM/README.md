...
Contrôle: **14 août 2026**. Anthropic confirme que **Workbench (legacy) sera retiré le 17 août** : les prompts, versions, complétions, fichiers et évaluations à conserver doivent être exportés avant cette date. Le Workbench actualisé est sans état et ne permet pas de réimporter l’export legacy [S167]. OpenAI confirme par ailleurs que **Health in ChatGPT reste réservé aux utilisateurs éligibles aux États-Unis** : la fonctionnalité n’est pas disponible en Belgique [S168–S169]. Le catalogue courant distingue modèles, applications, plateformes API, assistants de code et agents autonomes ; il ne constitue pas un plafond de suivi. Guide dédié : les **modèles exécutables en local selon le matériel**.
