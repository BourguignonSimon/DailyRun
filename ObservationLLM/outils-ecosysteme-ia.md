...
- **Le palier de service différé de Google Cloud est désormais chiffré.** La grille tarifaire, ouverte directement le 18 août, publie une colonne **Flex/Batch à 50 % du tarif Standard** : Gemini 3.1 Pro Preview de 2,00/12,00 à **1,00/6,00 USD/M** sous 200 k jetons, Gemini 3.7 Flash de 0,75/3,75 à **0,375/1,875 USD/M**. **Latence contractuelle et régions restent « non publié »**, `docs.cloud.google.com` étant refusé : la classe de service est arbitrable sur le prix, pas encore sur le délai [S196, S208].
- **Divergence tarifaire sur Claude Sonnet 5 entre Anthropic et Google Cloud.** L’API de premier rang reste à 2/10 USD/M, la hausse du 1er septembre y étant annulée ; la grille des modèles partenaires de Google Cloud affiche toujours **3,00/15,00 USD/M à compter du 1er septembre 2026**, et **3,30/16,50** sur les endpoints non globaux. Le fait établi est l’écart entre deux pages officielles lues le même jour, non l’intention de Google [S208, S209].
- **Claude Code 2.1.234, cinq correctifs de sécurité de plus** (17 août, 18:19 UTC). Le principal referme le vecteur de fuite d’identifiants **NTLM** sur les derniers accès fichiers **pré-approuvés**, en rejetant les chemins de l’espace de noms NT de Windows. Les diagnostics MCP cessent d’imprimer les secrets résolus. L’étiquette npm `stable` reste à **2.1.226**, huit versions en retrait [S210].
- **Le SDK Python OpenAI 3.2.0 ajoute un endpoint AWS Bedrock Runtime** (17 août). Fait de distribution, non de disponibilité : ni les modèles servis, ni les régions, ni la résidence des données ne sont publiés dans la source lue [S211].
