...
Contrôle: **19 août 2026**. L’état courant retient quatre faits établis en **source primaire durable**: la **grille tarifaire de Google Cloud**, qui aligne désormais **Claude Sonnet 5 sur 2,00/10,00 USD/M** en global et **2,20/11,00** en non global, sans plus aucune mention du 1er septembre 2026, qui chiffre la **prime de 10 % exactement des endpoints non globaux** et qui donne le **tarif du palier différé, à 50 % du tarif Standard**; **Claude Code 2.1.235**, publiée le 18 août à 18:24 UTC, qui resserre la chaîne de consentement des dialogues de permission; le **SDK Python OpenAI 3.3.0** du 18 août, qui ajoute des **endpoints de résidence de données nommés** et impose des dépendances réseau corrigées; et le **SDK Python OpenAI 3.0.0** qui impose **HTTPX2** en rupture de compatibilité, sa 3.1.0 **dépréciant l’API Videos** (arrêt annoncé au 24 septembre 2026). Côté poids ouverts, **Qwen3.8-27B** paraît sous **Apache 2.0** (dense, multimodal, 262 k de contexte) et **Qwen3.8-Max** sous une **licence propriétaire à partage de revenus**. L’état tarifaire courant retient **Gemini 3.7 Flash à 0,75/3,75 USD/M jusqu’au 31 décembre 2026**, tarif qui **double au 1er janvier 2027**, la disponibilité générale de **DeepSeek V4 Pro 0813 sous licence MIT**, la sortie de **GLM-5.3 sans ses poids**, et **Claude Sonnet 5 à 2/10 USD/M en tarif définitif sur les deux canaux**, l’API d’Anthropic comme la grille des modèles partenaires de Google Cloud, qui s’est alignée. Le comparatif couvre également **Mistral Regional Endpoints, Priority Tier et hébergement de modèles ouverts tiers**, **xAI Grok 4.6**, **OpenAI GPT-5.6-Cyber** et le palier **Ultrafast** pour GPT-5.6 Sol, ainsi que **Cohere North Mini Code** et **Gemini Omni Flash**. Voir [Prix API](#prix-api), [Exécution locale](#exécution-locale) et [Développement](#développement). Ce fichier remplace le classeur Excel demandé initialement: le dépôt a explicitement migré vers Markdown le 16 juillet 2026. Les « feuilles » sont les sections ci-dessous. Les références Sxx renvoient à [sources.md](sources.md).
