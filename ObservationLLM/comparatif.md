...
Contrôle: **18 août 2026**. L’état courant retient quatre faits établis en **source primaire durable**: la **grille tarifaire de Google Cloud**, de nouveau lisible, qui affiche encore **Claude Sonnet 5 à 3,00/15,00 USD/M au 1er septembre 2026** alors qu’Anthropic a annulé cette hausse sur son API de premier rang, qui chiffre la **prime de 10 % exactement des endpoints non globaux** et qui donne enfin le **tarif du palier différé, à 50 % du tarif Standard**; **Claude Code 2.1.234**, publiée le 17 août à 18:19 UTC avec cinq correctifs de sécurité sur des accès fichiers **pré-approuvés**; le **SDK Python OpenAI 3.2.0** du 17 août, qui ajoute un endpoint **AWS Bedrock Runtime**; et le **SDK Python OpenAI 3.0.0** qui impose **HTTPX2** en rupture de compatibilité, sa 3.1.0 **dépréciant l’API Videos** (arrêt annoncé au 24 septembre 2026). Côté poids ouverts, **Qwen3.8-27B** paraît sous **Apache 2.0** (dense, multimodal, 262 k de contexte) et **Qwen3.8-Max** sous une **licence propriétaire à partage de revenus**. L’état tarifaire courant retient **Gemini 3.7 Flash à 0,75/3,75 USD/M jusqu’au 31 décembre 2026**, tarif qui **double au 1er janvier 2027**, la disponibilité générale de **DeepSeek V4 Pro 0813 sous licence MIT**, la sortie de **GLM-5.3 sans ses poids**, et **Claude Sonnet 5 à 2/10 USD/M en tarif définitif sur l’API d’Anthropic — mais toujours annoncé à 3/15 USD/M au 1er septembre sur la grille de Google Cloud**. Le comparatif couvre également **Mistral Regional Endpoints, Priority Tier et hébergement de modèles ouverts tiers**, **xAI Grok 4.6**, **OpenAI GPT-5.6-Cyber** et le palier **Ultrafast** pour GPT-5.6 Sol, ainsi que **Cohere North Mini Code** et **Gemini Omni Flash**. Voir [Prix API](#prix-api), [Exécution locale](#exécution-locale) et [Développement](#développement). Ce fichier remplace le classeur Excel demandé initialement: le dépôt a explicitement migré vers Markdown le 16 juillet 2026. Les « feuilles » sont les sections ci-dessous. Les références Sxx renvoient à [sources.md](sources.md).
