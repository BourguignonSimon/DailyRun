...
Contrôle: **22 août 2026**. L’état courant retient trois faits établis chacun sur **deux canaux primaires indépendants**. Un, la **grille tarifaire de Google Cloud**, récupérée trois fois et lue par le balisage de ses onglets : la section « Anthropic’s Claude models » compte **sept volets de région**. Le volet **Global** facture Claude Sonnet 5 à **2,00/10,00 USD/M** et Opus 5 à 5,00/25,00 ; les volets **US et EU Multi-Region** facturent **+10 %** (Sonnet 5 à **2,20/11,00**, Opus 5 à 5,50/27,50) ; les **quatre volets monorégionaux**, dont **`europe-west1`**, ne contiennent **aucun modèle de génération 5** et s’arrêtent à la génération 4.x, elle aussi à +10 %. Deux, le régime de **résidence des données de l’API directe d’Anthropic** : `inference_geo` n’accepte que **« global »** et **« us »** (multiplicateur **1,1×**), sans aucune valeur européenne, et n’est accepté que sur Claude 4.6 et versions ultérieures — une requête portant ce paramètre sur Opus 4.5, Sonnet 4.5 ou Haiku 4.5 renvoie une **erreur 400** ; le stockage au repos dépend d’un réglage distinct, le **Workspace geo**. Trois, l’**outillage** : **Claude Code 2.1.239** (21 août, 17:18 UTC) fait apparaître la prime 1,1× dans `/cost` et corrige un doublement silencieux des appels facturés sur Bedrock derrière mandataire ; le **SDK Python 1.0** (20 août) bascule sa couche HTTP de **httpx à httpx2** et retire l’API Text Completions ainsi que `temperature`, `top_p` et `top_k` des méthodes Messages, tandis que le **SDK TypeScript reste en 0.120.0**.
