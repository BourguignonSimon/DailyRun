...
Contrôle: **20 août 2026**. L’état courant retient cinq faits établis en **source primaire durable**. Un, la **disponibilité générale des API Files et Skills** sur l’API Claude : les en-têtes `files-api-2025-04-14` et `skills-2025-10-02` ne sont plus requis, le format GA ouvre la pagination `page`/`next_page`, le filtre `ids[]` jusqu’à cent identifiants et un `expires_at` explicite, sous un quota publié de **1 To par organisation et 500 requêtes par minute**. Deux, la **restriction de domaines** des outils `web_search` et `web_fetch` des agents gérés, par `allowed_domains` et `blocked_domains` posés sur l’entrée de l’outil dans le tableau `configs` de `agent_toolset_20260401`, avec `max_content_tokens` et `user_location`. Trois, la **grille tarifaire de Google Cloud**, dont la lecture régionale est corrigée à ce contrôle : la section Anthropic n’a pas de colonne non globale mais deux colonnes de volume de contexte identiques, **Claude Sonnet 5 y valant 2,00/10,00 USD/M dans les sept régions publiées**, `europe-west1` comprise ; la prime de 10 % des endpoints non globaux et le tarif du palier différé à 50 % du tarif Standard demeurent, mais pour les modèles Gemini et le débit réservé. Quatre, **Claude Code 2.1.236 et 2.1.237**, publiées le 19 août à 18:45 et 23:57 UTC, qui durcissent la précédence des règles de refus de lecture du bac à sable macOS et corrigent le cache de prompt derrière une passerelle. Cinq, le **SDK Python OpenAI 3.3.1** du 19 août, montée de dépendances portant des correctifs de sécurité publiés, après les **endpoints de résidence de données nommés** de 3.3.0 ; le contexte de rupture reste la 3.0.0 qui impose **HTTPX2**, sa 3.1.0 **dépréciant l’API Videos** (arrêt annoncé au 24 septembre 2026). Côté poids ouverts, **Qwen3.8-27B** paraît sous **Apache 2.0** (dense, multimodal, 262 k de contexte) et **Qwen3.8-Max** sous une **licence propriétaire à partage de revenus**. L’état tarifaire courant retient **Gemini 3.7 Flash à 0,75/3,75 USD/M jusqu’au 31 décembre 2026**, tarif qui **double au 1er janvier 2027**, la disponibilité générale de **DeepSeek V4 Pro 0813 sous licence MIT**, la sortie de **GLM-5.3 sans ses poids**, et **Claude Sonnet 5 à 2/10 USD/M en tarif définitif sur les deux canaux**. Le comparatif couvre également **Mistral Regional Endpoints, Priority Tier et hébergement de modèles ouverts tiers**, **xAI Grok 4.6**, **OpenAI GPT-5.6-Cyber** et le palier **Ultrafast** pour GPT-5.6 Sol, ainsi que **Cohere North Mini Code** et **Gemini Omni Flash**. Voir [Prix API](#prix-api), [Exécution locale](#exécution-locale) et [Développement](#développement). Ce fichier remplace le classeur Excel demandé initialement: le dépôt a explicitement migré vers Markdown le 16 juillet 2026. Les « feuilles » sont les sections ci-dessous. Les références Sxx renvoient à [sources.md](sources.md).
