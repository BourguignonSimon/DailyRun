...
Contrôle: **16 août 2026**. L’état courant retient trois faits établis en **source primaire durable**: **Claude Code démarre en mode automatique** sur Pro, Max et Team depuis le 14 août, avec des planchers de version (2.1.228 sur macOS, Linux et WSL, **2.1.233** sur Windows natif) et cinq correctifs de sécurité livrés les 13 et 14 août; le **SDK Python OpenAI 3.0.0** impose **HTTPX2** en rupture de compatibilité et sa 3.1.0 **déprécie l’API Videos** (arrêt annoncé au 24 septembre 2026); le **palier de service différé** devient publiquement disponible sur Vertex AI, sans tarif ni région publiés. Côté poids ouverts, **Qwen3.8-27B** paraît sous **Apache 2.0** (dense, multimodal, 262 k de contexte) et **Qwen3.8-Max** sous une **licence propriétaire à partage de revenus**. L’état tarifaire courant retient **Gemini 3.7 Flash à 0,75/3,75 USD/M jusqu’au 31 décembre 2026**, tarif qui **double au 1er janvier 2027**, la disponibilité générale de **DeepSeek V4 Pro 0813 sous licence MIT**, la sortie de **GLM-5.3 sans ses poids**, et le maintien de **Claude Sonnet 5 à 2/10 USD/M en tarif définitif**. Le comparatif couvre également **Mistral Regional Endpoints, Priority Tier et hébergement de modèles ouverts tiers**, **xAI Grok 4.6**, **OpenAI GPT-5.6-Cyber** et le palier **Ultrafast** pour GPT-5.6 Sol, ainsi que **Cohere North Mini Code** et **Gemini Omni Flash**. Voir [Prix API](#prix-api), [Exécution locale](#exécution-locale) et [Développement](#développement). Ce fichier remplace le classeur Excel demandé initialement: le dépôt a explicitement migré vers Markdown le 16 juillet 2026. Les « feuilles » sont les sections ci-dessous. Les références Sxx renvoient à [sources.md](sources.md).

> ⚠️ **Niveau de preuve de cette édition.** La politique réseau sortante bloque la majorité des domaines éditeurs et **la totalité des six points d’entrée de veille quotidienne**: sur **52 sources retestées, 4 répondent**. **Deux canaux de preuve primaire ont été ouverts pour la première fois ce contrôle** — `code.claude.com` et `raw.githubusercontent.com` — ce qui établit en source primaire durable les lignes Claude Code, SDK OpenAI et SDK Google. Les tarifs Claude restent établis en source primaire ouverte directement. Les autres lignes sont établies par convergence de sources indépendantes et **plafonnées à 89/100**, ou conservent le niveau de preuve de leur dernier contrôle effectif. **Angles morts**: `huggingface.co` et `modelscope.cn` — aucune licence de poids ouverts n’est lisible à la source — et les pages tarifaires de Google Cloud et d’OpenAI. Voir [sources.md](sources.md). Revérification recommandée avant tout engagement contractuel ou budgétaire.
...
| **DeepSeek V4 Pro 0813** | 1 M | 0,435 USD miss | 0,003625 USD hit | 0,87 USD | 0,38 € | 0,75 € | Non publié | **Disponibilité générale depuis le 13/08**, poids **MIT** publiés (~893 Go); MoE 1,6 T / 49 B actifs, sortie max 384 k. **V4-Flash 0731** reste disponible (284 B, 0,14 USD/M entrée, Responses API, adapté Codex). **Aucune région UE annoncée** pour l’API directe | S66–S67, S128, S182–S183 |
