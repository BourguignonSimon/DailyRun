...
Contrôle : **21 août 2026**. Ce référentiel rassemble les URL de base à privilégier à chaque recherche quotidienne. L’indice mesure l’aptitude de la source à établir les sujets indiqués ; il ne constitue pas une note universelle de vérité.

> **Accessibilité mesurée le 21 août 2026.** Les **55 sources du référentiel ont été retestées une à une** depuis l’environnement d’exécution : **cinq répondent pleinement** — R02 (documentation de la plateforme Claude), R38 (PyPI), R40 (blog du Model Context Protocol), R39 (API du registre npm) et R55 (API GitHub, métadonnées) — **deux répondent partiellement** — R04 (Google Cloud), dont l’hôte `cloud.google.com` s’ouvre pour la page tarifaire de l’IA générative alors que le chemin `/vertex-ai/generative-ai/docs/` et le sous-domaine `docs.cloud.google.com` restent refusés, et R54 (contenu brut des dépôts publics GitHub, accessible sur les chemins de fichiers mais refusé sur l’hôte nu) — et **quarante-huit sont refusées** au tunnel CONNECT du mandataire, qui répond 403 à la requête CONNECT, dont la totalité des six points d’entrée de veille quotidienne obligatoires. Onzième contrôle consécutif au même profil général : c’est une caractéristique stable de l’environnement, pas un incident.
>
