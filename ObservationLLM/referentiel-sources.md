...
Contrôle : **18 août 2026**. Ce référentiel rassemble les URL de base à privilégier à chaque recherche quotidienne. L’indice mesure l’aptitude de la source à établir les sujets indiqués ; il ne constitue pas une note universelle de vérité.

> **Accessibilité mesurée le 18 août 2026.** Les **55 sources du référentiel ont été retestées une à une** depuis l’environnement d’exécution : **six répondent pleinement** — R02 (documentation de la plateforme Claude), R38 (PyPI), R40 (blog du Model Context Protocol), R53 (documentation et journal de version de Claude Code), R54 (contenu brut des dépôts publics GitHub, sur les chemins de fichiers) et R55 (API GitHub, dépôts en périmètre de session) — **deux répondent partiellement** — R04 (Google Cloud), dont l’hôte `cloud.google.com` s’ouvre de nouveau pour la page tarifaire de l’IA générative alors que `docs.cloud.google.com` reste refusé, et R39 (npm), dont l’interface web `www.npmjs.com` renvoie 403 quand l’API `registry.npmjs.org` s’ouvre — et **quarante-sept sont refusées** au tunnel CONNECT du mandataire, qui répond 403 à la requête CONNECT, dont la totalité des six points d’entrée de veille quotidienne obligatoires. Huitième contrôle consécutif au même profil général : c’est une caractéristique stable de l’environnement, pas un incident.
>
> **Portée et limites des huit canaux primaires.** R02 a établi ce jour l’annulation de la hausse de Claude Sonnet 5 sur l’API de premier rang et la mention « retired, except on Bedrock and Google Cloud » qui atteste que les clouds partenaires suivent leurs propres calendriers. **R04, rouvert à ce contrôle**, a établi la grille tarifaire complète de Google Cloud : la ligne Claude Sonnet 5 encore programmée à la hausse au 1er septembre, la prime de 10 % exactement des endpoints non globaux jusqu’aux unités de débit réservé, et le tarif du palier différé, à 50 % du tarif Standard. R53 et R39 ont établi Claude Code 2.1.234 et son horodatage ; R54 et R38, le contenu du SDK Python OpenAI 3.2.0. Limite de R54 : elle donne accès aux **fichiers versionnés** d’un dépôt, non aux publications de version, tickets ou discussions — c’est pourquoi **R30 reste hors d’atteinte** malgré R54. Limite de R04 : seules les pages tarifaires répondent, la documentation de résidence demeure refusée.
>
