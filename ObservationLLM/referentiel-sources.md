...
Contrôle : **20 août 2026**. Ce référentiel rassemble les URL de base à privilégier à chaque recherche quotidienne. L’indice mesure l’aptitude de la source à établir les sujets indiqués ; il ne constitue pas une note universelle de vérité.

> **Accessibilité mesurée le 20 août 2026.** Les **55 sources du référentiel ont été retestées une à une** depuis l’environnement d’exécution : **six répondent pleinement** — R02 (documentation de la plateforme Claude), R38 (PyPI), R40 (blog du Model Context Protocol), R53 (documentation et journal de version de Claude Code), R54 (contenu brut des dépôts publics GitHub, sur les chemins de fichiers) et R39 (API du registre npm) — **deux répondent partiellement** — R04 (Google Cloud), dont l’hôte `cloud.google.com` s’ouvre pour la page tarifaire de l’IA générative alors que le chemin `/vertex-ai/generative-ai/docs/` et le sous-domaine `docs.cloud.google.com` restent refusés, et R55 (API GitHub), dont la racine et `/rate_limit` répondent quand les endpoints de dépôt sont refusés — et **quarante-sept sont refusées** au tunnel CONNECT du mandataire, qui répond 403 à la requête CONNECT, dont la totalité des six points d’entrée de veille quotidienne obligatoires. Dixième contrôle consécutif au même profil général : c’est une caractéristique stable de l’environnement, pas un incident.
>
> **Portée et limites des huit canaux primaires.** R02 porte le fait dominant du jour : ses **notes de version** datent au 19 août 2026 la disponibilité générale des API **Files** et **Skills**, celle des endpoints de gestion des utilisateurs de l’Admin API, la restriction de domaines des outils web des agents gérés, la mémoire attachable en bac à sable auto-hébergé et la refonte de la visionneuse de sessions ; sa **documentation de l’API Files** confirme séparément l’abandon de l’en-tête bêta et détaille la double lecture selon que l’en-tête est envoyé ou non ; sa page tarifaire répète que la hausse de Claude Sonnet 5 n’aura pas lieu. **R04, ouvert pour le troisième run consécutif**, a établi la grille tarifaire de Google Cloud et permis d’en **corriger la lecture régionale** : les deux colonnes de la section Anthropic distinguent des volumes de contexte et non des régions, et un décompte positionnel des occurrences de « non-global », borné aux limites de cette section, n’en trouve aucune — la prime de 10 % relevée sur la page appartient à la section Gemini et à celle du débit réservé. R54 et R39 ont établi Claude Code 2.1.236 et 2.1.237 et leurs horodatages ; R54 et R38, le contenu du SDK Python OpenAI 3.3.1 et celui des SDK Anthropic Python 0.124.0/0.125.0 ; R54 seul, le journal du SDK TypeScript 0.119.0/0.120.0, qui corrobore la sortie de bêta. R38 et R39 ont par ailleurs servi à contrôler cinquante-trois paquets du catalogue d’outils. R40 n’a **aucune publication nouvelle** depuis le 28 juillet. Limite de R54 : elle donne accès aux **fichiers versionnés** d’un dépôt, non aux publications de version, tickets ou discussions — c’est pourquoi **R30 reste hors d’atteinte** malgré R54. Limite de R04 : seules les pages tarifaires répondent, la documentation de résidence demeure refusée. Limite de R38 et R39 : un registre prouve qu’une version existe et à quelle date, jamais ce qu’elle change.
