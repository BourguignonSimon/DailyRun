...
Contrôle: **16 août 2026**. État courant, avec trois faits nouveaux établis en **source primaire durable**: Claude Code démarre en mode automatique sur Pro, Max et Team depuis le 14 août, avec des planchers de version et cinq correctifs de sécurité livrés les 13 et 14 août; le SDK Python OpenAI passe en 3.0.0 avec une rupture de compatibilité HTTPX2 et sa 3.1.0 déprécie l’API Videos, dont l’arrêt est annoncé au 24 septembre 2026; la Compliance API Anthropic couvre les transcriptions de sessions Cowork et Claude Code. Côté poids ouverts, l’attente Qwen est tranchée dans les deux sens: Qwen3.8-27B paraît sous Apache 2.0, Qwen3.8-Max sous une licence propriétaire à partage de revenus. Par ailleurs, Google publie Gemini 3.7 Flash à 0,75/3,75 USD/M jusqu’au 31 décembre 2026 avec doublement au 1er janvier 2027 et ouvre publiquement le palier de service différé sur Vertex AI, Z.ai publie GLM-5.3 sans ses poids ouverts, DeepSeek V4 Pro 0813 passe en disponibilité générale sous licence MIT, et le tarif de Claude Sonnet 5 reste à 2/10 USD/M à titre définitif.
...
>
> ✅ **Voie de preuve primaire disponible — deux canaux gagnés ce contrôle.** `code.claude.com` (documentation, digest hebdomadaire et journal de version de Claude Code) et `raw.githubusercontent.com` (contenu brut des dépôts publics, donc journaux de version et fichiers de licence des éditeurs, alors que `github.com` reste refusé) ont été ouverts directement pour la première fois. Joints à la documentation Anthropic (`platform.claude.com`), au blog du Model Context Protocol et aux registres `pypi.org` et `registry.npmjs.org`, ils ont été ouverts
...
La règle appliquée ici est de refléter l’état juridique **vérifié**, pas l’état annoncé: **xAI (rang 8) et Anysphere/Cursor (rang 16) restent deux entrées distinctes**. Fusionner sur la foi d’une clôture annoncée reviendrait à publier comme fait ce qui reste une prévision — exactement l’erreur que ce livrable s’est interdite pendant plusieurs semaines sur les poids ouverts de Qwen3.8-Max, dont le contrôle du 16 août montre qu’elle aurait été double: les poids ont fini par paraître, mais **sous une licence qui n’est pas celle qu’on aurait présumée**. Mais la conséquence doit être anticipée: **à la clôture, la sélection passera mécaniquement à 19 entrées et une place se libérera.**
...
**Un signal de sélection enfin tranché — et dans les deux sens.** Alibaba avait publié **Qwen3.8-Max** à parité tarifaire avec GPT-5.6 Terra (2/6 USD/M) tout en annonçant l’ouverture des poids d’un modèle de classe « Max », une première [S115]. L’attente, suivie run après run, se solde le **16 août** par un résultat en deux temps qu’il faut lire précisément. **Les poids de classe Max existent** — point de contrôle Qwen3.8-2.4T-A95B publié le 12 août — **mais sous une licence propriétaire à partage de revenus, pas sous Apache 2.0**: concession de style MIT, attribution obligatoire au-delà de 100 M d’utilisateurs actifs mensuels ou 20 M USD de revenu mensuel, licence payante pour les activités de modèle en tant que service au-delà de 50 M USD sur douze mois glissants, usage strictement interne excepté; et le point de contrôle est **uniquement textuel**, sans le contexte de 1 M du produit hébergé [S198]. **En revanche, Qwen3.8-27B paraît le 14 août sous Apache 2.0** — dense, nativement multimodal, 262 144 jetons de contexte natif — et c’est là que se trouve la valeur réelle pour un utilisateur belge [S197]. La leçon de méthode est double. D’une part, l’écart entre l’annonce et la livraison, qui avait servi d’indicateur de fiabilité, se résorbe: l’engagement a été tenu, avec retard. D’autre part, **il a été tenu sur la lettre et non sur l’esprit**: « ouverture des poids » ne signifiait pas « licence permissive », et la clause générale du dépôt public de l’éditeur — « All our open-weight models are licensed under Apache 2.0 » — **contredit** la licence rapportée pour Max [S199]. Cette contradiction n’est pas résolue: `huggingface.co` et `modelscope.cn` étant refusés, aucun fichier de licence n’a été lu à la source et les deux faits restent plafonnés. Le poids relatif d’Alibaba dans la sélection augmente néanmoins, porté par le 27B et non par le Max.
...
**Le fait marquant est tarifaire autant que technique.** À 2/6 USD/M, Qwen3.8-Max se place **en parité d’entrée avec GPT-5.6 Terra** (2/12), avec une sortie moitié moins chère. C’est la première fois qu’un modèle chinois de classe frontière s’aligne frontalement sur la grille d’un modèle occidental de référence plutôt que de se positionner comme alternative économique. Côté poids, l’ouverture annoncée pour la semaine du 10 août a eu lieu avec retard et en deux temps: **Qwen3.8-2.4T-A95B le 12 août sous licence propriétaire Qwen3.8-Max à partage de revenus**, point de contrôle uniquement textuel de 2 400 milliards de paramètres pour 95 milliards actifs — sans portée d’auto-hébergement réelle et à réserver aux revues de conformité [S198]; et **Qwen3.8-27B le 14 août sous Apache 2.0**, dense, nativement multimodal texte, image et vidéo en entrée, 262 144 jetons de contexte natif, extension vers ~1 M par YaRN annoncée [S197]. C’est ce second modèle qui change quelque chose en pratique: une licence sans seuil de revenu sur un dense de 27 milliards de paramètres vise les configurations à 24 Go de mémoire vidéo, avec déchargement partiel envisageable sur 12 Go. Réserve de méthode qui commande la prudence: les dépôts de poids sont refusés depuis l’environnement d’exécution, **aucun fichier de licence n’a été lu à la source**, aucune variante quantifiée n’a été mesurée et la qualité en français et en néerlandais n’est pas évaluée. Traitez ces deux faits comme établis par convergence et plafonnés, et vérifiez la licence dans le dépôt avant tout engagement.
...
6. **Licences de poids ouverts non lues à la source.** Les poids de Qwen3.8-Max et de Qwen3.8-27B ont été publiés les 12 et 14 août, mais `huggingface.co` et `modelscope.cn` sont refusés par la politique réseau sortante: **aucun fichier de licence n’a pu être lu à la source**. La licence propriétaire à partage de revenus rapportée pour Qwen3.8-Max contredit la clause générale du dépôt public de l’éditeur, qui affirme qu’Apache 2.0 s’applique à tous ses modèles à poids ouverts [S199]. Cette contradiction reste non résolue et les deux faits restent plafonnés à 89/100 [S197–S199].
