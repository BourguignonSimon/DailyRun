...
30. Le dashboard public ouvre sur une page d’accueil courte orientée tâches. Dès le premier écran, un nouveau visiteur doit comprendre sans jargon que le site centralise les nouveautés utiles de l’IA : modèles et API, outils et agents, annonces et retraits, prix, choix par usage, sécurité, régions européennes et exécution locale. Maintiens une promesse explicite, un accès prioritaire aux nouveautés et au choix de modèle, les marqueurs « mise à jour quotidienne », « sources officielles prioritaires », « confiance et limites affichées » et « accès public sans compte », puis six accès rapides vers Actualités, Choisir, Outils, Localisation, LLM locaux et Sources. Conserve le mode expert exhaustif. Toutes les vues utilisent le dernier run validé, des liens accessibles sans connexion GitHub, une navigation cohérente, des filtres utilisables au clavier, un libellé de confiance et les limites de vérification.
