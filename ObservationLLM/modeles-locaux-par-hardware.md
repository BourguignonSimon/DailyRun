...
Contrôle: **14 août 2026**. Point de vue: particulier ou développeur résidant en Belgique qui veut faire tourner un modèle **sur sa propre machine**, sans envoyer de données à un fournisseur cloud. Ce fichier complète l'[observatoire des 20 écosystèmes](README.md); il ne le remplace pas.

> ⚠️ **État de vérification au 14 août 2026.** Aucune carte modèle, licence ou mesure de vitesse citée dans ce guide n’a pu être revérifiée ce jour : `huggingface.co`, `ollama.com`, `lmstudio.ai` et les domaines fournisseurs restent bloqués par la politique de sortie réseau. Les tableaux ci-dessous conservent la vérification de leur dernier contrôle effectif et sont explicitement marqués **non revérifiés à cette date**. Seuls les registres de distribution officiels (`pypi.org`) sont ouverts directement et prouvent des versions de bibliothèques d’exécution, pas des mesures de vitesse ni des licences.
...
- Le paysage local évolue vite; ce fichier reflète l'état au **14 août 2026** et sera revu à chaque exécution. La correction Gemma 3 → Gemma 4 et Qwen3 → Qwen3.6 opérée ce mois-ci montre concrètement le risque: **une famille de modèles locaux peut être remplacée en quelques mois sans que les tutoriels et registres cessent de citer l'ancienne**. Vérifiez toujours la date de la carte de modèle, pas seulement le nom.
